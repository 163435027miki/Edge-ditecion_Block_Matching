...
[ユーティリティルーチン]
------------------------------------------------------------------------------------------
　nrerror は致命的なエラーが生じたとき呼び出す．このルーチンは適当なエラーメッセージを
出力してプログラムの実行を終了する．
　他のルーチンは，ベクトル，行列の記憶領域を割り当てたり，解除したりするものである．
本書の記憶領域割り当て，解除はすべてこれらのルーチンを通して行われる．従って，記憶域の
割り当て方を変えたいなら，これらのルーチンだけを書き換えればよい．
------------------------------------------------------------------------------------------
	|
...
{ 
	...
	/* allocate rows aud set pointers to them */ 
...
/* allocate a double matrix with subscript range m [nrl . .nrh] [ncl ..nch] */ 
{ 
	long i , nrow=nrh-nrl+i,ncol=nch-ncl+1; 
	double **m; 
...
void free_lvector(unsigned long *v, long nl , long uh) 
/* free an unsigned long vector allocated with lvector() */
{ 
	free((FREE_ARG) (v+nl-NR_END)) ; 
} 

void free_dvector(double *v, long nl , long uh) 
...
/* free a submatrix allocated by subiuatrix() */ 
{ 
...
void free_i3tensor(int ***t, long nrl, long nrh, long ncl, long nch, 
	long ndl , long ndh) 
/* free a int 3tensor allocated by i3tensor() */ 
{ 
	free((FREE_ARG) (t[nrl][ncl]+ndl-NR_END) ) ; 
	free((FREE_ARG) (t[nrl]+ncl-NR_END)); 
	free((FREE_ARG) (t+nrl-NR_END)) ; 
...
/* allocate an int vector with subscript rauge v[nl. .nh] */
{ 
	int *v; 
...
long nh,nl; 
/* allocate a double vector with subscript range v[nl. .uh] */ 
{ 
	double *v; 
...
{ 
	...
	if (!m) nrerror("allocation failure i in matrix()") ; 
	m += NR_END ; 
	m -= nrl; 
...
long nch,ncl ,nrh,nrl ; 
/* allocate a double luatrix with subscript range m[nrl. .nrh] [ncl . .nch] */ 
{ 
	long i , nrow=nrh-nrl+1,ncol=nch-ncl+1; 
	double **m; 
...
{ 
	...
	if (!t[nrl][ncl]) nrerror("allocatlon fallure 3 in f3tensor()") ;
	t[nrl][ncl] += NR_END; 
	t[nrl][ncl] -= ndl ; 
...
{ 
	...
	if (!t[nrl][ncl]) nrerror("allocatlon fallure 3 in i3tensor()") ;
	t[nrl][ncl] += NR_END; 
	t[nrl][ncl] -= ndl ; 
...
double **m; 
long nch,ncl ,nrh,nrl ; 
/* free a double matrix allocated by dnatrix() */
{ 
	free((FREE_ARG) (m[nrl]+ncl-NR_END) ) ; 
	free((FREE_ARG) (m+nrl-NR_END)) ; 
}
...
void free_f3tensor(t , nrl , nrh, ncl , nch , ndl ,ndn) 
double ***t; 
long nch , ncl , ndn , ndl , nrh , nrl ; 
/* free a double 3tensor allocated by f3tensor() */
{ 
	free((FREE_ARG) (t[nrl][ncl]+ndl-NR_END)) ; 
	free((FREE_ARG) (t[nrl]+ncl-NR_END)) ; 
	free((FREE_ARG) (t+nrl-NR_END)) ; 
...
void free_i3tensor(t , nrl , nrh, ncl , nch , ndl ,ndn) 
int ***t; 
long nch , ncl , ndn , ndl , nrh , nrl ; 
/* free a double 3tensor allocated by i3tensor() */
{ 
	free((FREE_ARG) (t[nrl][ncl]+ndl-NR_END)) ; 
	free((FREE_ARG) (t[nrl]+ncl-NR_END)) ; 
	free((FREE_ARG) (t+nrl-NR_END)) ; 
